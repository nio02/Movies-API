...
public interface IMovieRepository
{
    ...
    Task<bool> UpdateAsync(Movie movie, CancellationToken token = default, Guid? userId = default);
    Task<bool> DeleteByIdAsync(Guid id, CancellationToken token = default, Guid? userId = default);
    Task<bool> ExistsByIdAsync(Guid id, CancellationToken token = default, Guid? userId = default);
}